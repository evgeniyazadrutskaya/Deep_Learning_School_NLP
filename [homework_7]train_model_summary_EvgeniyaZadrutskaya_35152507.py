...
def train(model, iterator, optimizer, criterion, clip, train_history=None, valid_history=None):
    ...
    for i, batch in enumerate(iterator):
        ...
        # YOUR CODE
        inputs = batch['inputs']
        targets = batch['outputs'] # [batch_size, seq num]
    
        optimizer.zero_grad()
        outputs = model(inputs) # [batch_size, seq num]

        # 1 / outputs... or just -outputs... or 0
        outputs_ = torch.cat((-outputs.unsqueeze(2), outputs.unsqueeze(2)), dim=2) 
        # [batch_size, seq num, 2]
        loss = criterion(outputs_.permute(0, 2, 1), targets)
